...
def create_app():
    ...
    with gr.Blocks(title="🤖 AI Agent - Sandboxed Local Assistant") as app:
        # Add JavaScript to prevent browser from opening dragged files
        gr.HTML("""
        <script>
...
- Lahat ng blocked ops ay i-log
        ...
        def start_chat(message, history, api_key, model, file_paths, is_running):
            ...
            if is_running:
                return history, "", False, gr.update(visible=False), gr.update(visible=False)
            
            # Show loading state
            yield history, "", True, gr.update(visible=False, interactive=False), gr.update(visible=True)
            
            # Run chat stream
            for result in chat_stream(message, history, api_key, model, file_paths):
                yield result[0], "", True, gr.update(visible=False, interactive=False), gr.update(visible=True)
            
            # Reset state after completion
            yield result[0], "", False, gr.update(visible=True, interactive=True), gr.update(visible=False)
        ...
        send_btn.click(
            ...
            outputs=[chatbot, msg, is_running_state, send_btn, stop_btn]
        )
